...
window.addEventListener("load", () => {
    ...
    function hideOverlay() {
        ...
        publish(
            new UiMessage.Notify.OverlayVisibilityChanged(
                overlayVisibility,
                overlayId
            )
        );
    ...
    function changePage(paginationListElement: any, page: number) {
        const recordsPerPage: number =
            paginationListElement.data("records-per-page");
        const numOfItems: number =
            paginationListElement.children(".item").length;
    ...
    consume(UiMessage.Set.CurrentStackInfo, (event) => {
        ...
        $(".stack-nav-previous").toggleClass(
            "disabled",
            !stackInfo?.canNavigateToPrevious
        );
        $(".stack-nav-next").toggleClass(
            "disabled",
            !stackInfo?.canNavigateToNext
        );
    });
    ...
    $(".analytics-nav").on("change", (e) => {
        if (e.target === $(".analytics-nav")[0]) {
            publish(
                new UiMessage.Notify.TabChanged(
                    (<any>e.originalEvent).detail.id
                )
            );
        }
    });
    ...
    $(document).on("click", ".refresh-link", function () {
        publish(new UiMessage.Notify.TabRefreshRequested(true));
    });
    ...
    $(document).on("click", ".duration-histogram-link", function () {
        ...
        const spanInstrumentationLibrary = $(this).data(
            "span-instrumentationlib"
        );

        publish(
            new UiMessage.Notify.OpenDurationHistogramPanel(
                spanName,
    ...
    $(document).on("click", ".scaling-histogram-link", function () {
        ...
        publish(
            new UiMessage.Notify.OpenScalingHistogramPanel(
                spanName,
                spanInstrumentationLibrary
            )
        );
    ...
    $(document).on("click", ".custom-start-date-recalculate-link", function () {
        ...
        $timeInfo.find(".custom-start-date-refresh-link").show();
        $timeInfo.show();

        (<any>$this.closest("li.list-item-menu")).superfish("hide");

        publish(
            new UiMessage.Notify.SetInsightCustomStartTime(
                codeObjectId,
                insightType,
                new Date()
            )
    ...
    $(document).on("click", ".trace-link", function () {
        ...
        publish(
            new UiMessage.Notify.OpenTracePanel(
                traceIds,
                traceLabels,
                span,
                jaeger
    ...
    $(document).on("click", ".jaeger-link", function () {
        ...
        publish(
            new UiMessage.Notify.OpenJaegerPanel(
                traceIds,
                traceLabels,
                span,
                jaeger
    ...
    publish(
        new UiMessage.Notify.TabLoaded($(".analytics-nav").attr("activeid"))
    );
});
