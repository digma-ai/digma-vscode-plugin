    ...
    constructor(
        ...
        this._disposables.push(
            vscode.languages.registerHoverProvider(
                ...
                new MethodCallErrorHoverProvider(
                    documentInfoProvider,
                    codeInspector,
                    workspaceState
                )
        ...
        this._disposables.push(
            vscode.commands.registerCommand(
                MethodCallErrorTooltip.Commands.ShowErrorView,
                async (args) => {
                    await vscode.commands.executeCommand(
                        ErrorsViewTab.Commands.ShowErrorView,
                        args.codeObjectId,
                        args.codeObjectDisplayName,
                        args.errorFlowId
                    );
                }
            )
...
class MethodCallErrorHoverProvider implements vscode.HoverProvider {
    ...
    public async provideHover(
        ...
        const sourceDocInfo = await this._documentInfoProvider.getDocumentInfo(
            document
        );
        if (!sourceDocInfo) {
            return;
        ...
        let methodInfo: MethodInfo | undefined =
            sourceDocInfo?.methods.firstOrDefault(
                (m) => m.nameRange?.contains(position) ?? false
            );
        let remoteDoc: DocumentInfo | undefined = undefined;
        if (!methodInfo) {
            if (
                !sourceDocInfo.tokens.some(
                    (t) =>
            ...
            remoteDoc = await this._codeInspector.getDocumentInfo(
                document,
                position,
                this._documentInfoProvider
            );
    ...
    private async getMethodMarkdown(
        methodInfo: MethodInfo,
        remoteDoc: DocumentInfo
        ...
        if (importantInsights.length > 0) {
            markdown.appendMarkdown(
                ` <span style="color:#FF0000;"><i>${importantInsights
                    .map((x) => x.name)
                    .join(" ")}</i></span>`
            );
            markdown.appendText("\n");
        ...
        for (var stat of perfStats) {
            const durationInsight: SpanDurationsInsight =
                stat as SpanDurationsInsight;

            const p50 = durationInsight.percentiles
            ...
            if (p50 || p95) {
                ...
                if (p50 != null) {
                    markdown.appendText(
                        `${p50.currentDuration.value} ${p50.currentDuration.unit} (Median) `
                    );
                }

                if (p95 != null) {
                    markdown.appendText(
                        `${p95.currentDuration.value} ${p95.currentDuration.unit} (P95) `
                    );
                }
                markdown.appendText("\n");
            }
        ...
        const errors =
            await this._documentInfoProvider.analyticsProvider.getCodeObjectsErrors(
                methodInfo.idsWithType
            );
        if (!errors?.length) {
            return;
        }
        ...
        for (const error of errors) {
            markdown.appendMarkdown(`- \`${error.name}\``);
            markdown.appendMarkdown(
                ` \u00B7 <span style="color:#cca700;"><i>${error.characteristic}</i></span>`
            );
            const command = MethodCallErrorTooltip.Commands.ShowErrorView;
            const args = encodeURIComponent(
                JSON.stringify({
                    codeObjectId: methodInfo.symbol.id,
                    codeObjectDisplayName: methodInfo.displayName,
                    errorFlowId: error.uid
                })
            );
            markdown.appendMarkdown(
                ` \u00B7 [$(link-external)](command:${command}?${args} "Show in side panel") `
            );
            markdown.appendText("\n");
        }
