...
export class FastapiEndpointExtractor implements IEndpointExtractor {
    async extractEndpoints(
        ...
        if (
            !tokens.some(
                (t) => t.text == "fastapi" && t.type == TokenType.module
            )
        ) {
            return [];
        }
        ...
        for (let i = 0; i < tokens.length - 1; i++) {
            ...
            if (
                appToken.type === TokenType.variable &&
                methodToken.type === TokenType.class &&
                methodToken.text === "APIRouter"
            ) {
                const routerLine = document.getText(
                    new vscode.Range(
                ...
                const match = new RegExp(
                    `^${appToken.text}\\s*=\\s*\\APIRouter\\s*\\((.*=.*,\\s*)*prefix=\\s*["'](.*?)["'](ֿֿֿ\\s*,.*=.*)*\\)*\\)`
                ).exec(routerLine);
                if (match) {
                    prefix = match[2];
                }
            ...
            if (
                (appToken.text != "app" && appToken.text != "router") ||
                appToken.type != TokenType.variable ||
                methodToken.type != TokenType.method
            ) {
                continue;
            }
            ...
            if (!match) {
                ...
                match = new RegExp(
                    `^(app|router)\\.${method}\\((.*=.*,\\s*)*path=\\s*["'](\\/.*?)["'](ֿֿֿ\\s*,.*=.*)*\\)`
                ).exec(lineText);
                index = 3;
            }
            ...
            const relevantFunc = symbolInfo.firstOrDefault((s) =>
                s.range.contains(methodToken.range)
            );
            if (!relevantFunc) {
                continue;
            }
            ...
            const folderPrefix = folder?.uri.path
                .split("/")
                .slice(0, -1)
                .join("/");
            const relevantPath = document.uri.path.substring(
                folderPrefix!.length
            );
