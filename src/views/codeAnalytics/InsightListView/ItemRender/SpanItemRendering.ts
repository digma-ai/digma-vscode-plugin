...
export class SpanItemHtmlRendering {
    ...
    public getBestUnit(previousDuration: Duration, currentDuration: Duration) {
        const change = moment.duration(
            Math.abs(previousDuration.raw - currentDuration.raw) / 1000000,
            "ms"
        );
    ...
    private findByPercentileAndPeriod(
        insight: SpanDurationsInsight,
        percentile: number,
        period: string
    ) {
        return insight.periodicPercentiles
            ?.filter((x) => x.percentile === percentile && period === period)
            .firstOrDefault()?.currentDuration.value;
    }

    public spanDurationItemHtml(
        insight: SpanDurationsInsight
    ): InsightTemplateHtml {
        ...
        for (const item of insight.percentiles) {
            ...
            percentileHtmls.push(
                /*html*/ `<span>${item.currentDuration.value} ${item.currentDuration.unit}</span>`
            );
            if (item.previousDuration && item.changeTime) {
                const rawDiff = Math.abs(
                    item.currentDuration.raw - item.previousDuration.raw
                );
                changeMeaningfulEnough =
                    rawDiff / item.previousDuration.raw > 0.1 &&
                    rawDiff > tolerationConstant;
                if (changeMeaningfulEnough) {
                    const verb =
                        item.previousDuration.raw > item.currentDuration.raw
                            ? "dropped.png"
                            : "rose.png";
            ...
            if (
                item.changeTime &&
                changeMeaningfulEnough &&
                item.changeVerified === false
            ) {
                percentileHtmls.push(
                    /*html*/ `<span title="This change is still being validated and is based on initial data.">Evaluating</span>`
                );
            } else {
                percentileHtmls.push(/*html*/ `<span></span>`);
            }
