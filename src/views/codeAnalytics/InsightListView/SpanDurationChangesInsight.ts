...
export class SpanDurationChangesInsightCreator
    implements IInsightListViewItemsCreator
{
    ...
    public async create(
        codeObjectsInsight: SpanDurationChangesInsight[]
        ...
        const spans = codeObjectInsight.spanDurationChanges.map((x) => x.spanInfo || x.span);

        const hints = this._spanLinkResolver.codeHintsFromSpans(spans);

        const spanLocations =
            await this._spanLinkResolver.searchForSpansByHints(hints);

        await codeObjectInsight.spanDurationChanges.forEach(
            async (spanChange, index) => {
                const changedPercentiles = spanChange.percentiles
                    .filter((x) => x.changeTime && x.previousDuration)
                    .firstOrDefault();
                let detailsHtml = "";
                let unverified = "";
                ...
                if (
                    changedPercentiles.previousDuration &&
                    changedPercentiles.changeTime &&
                    Math.abs(
                        changedPercentiles.currentDuration.raw -
                            changedPercentiles.previousDuration.raw
                    ) /
                        changedPercentiles.previousDuration.raw >
                        0.1
                ) {
                    ...
                    detailsHtml = /*html*/ `<span class="change"> 
                                                    <img class="insight-main-image" style="align-self:center;" src="${this._viewUris.image(
                                                        verb
                                                    )}" width="8" height="8"> 
                                                    ${renderer.getBestUnit(
                                                        changedPercentiles.previousDuration,
                                                        changedPercentiles.currentDuration
                                                    )}, ${changedPercentiles.changeTime.fromNow()}</span>`;

                    const result = spanLocations[index];
                    const html = ` 
                <div class="summary-list-item">
                    <div class="list-item-content-area">
                        <div class="span-name flex-v-center ${
                            result ? "link" : ""
                        }" data-code-uri="${
                        result?.documentUri
                    }" data-code-line="${result?.range.end.line! + 1}">
                            ${
                                spanChange.spanInfo?.name ||
        ...
        const html = new GlobalInsightListTemplate().getInsightTemplate(
            "Performance Changes",
            spanDurationHtml
        );
