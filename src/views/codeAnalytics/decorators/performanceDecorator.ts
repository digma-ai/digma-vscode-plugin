...
export class PerformanceDecorator implements vscode.Disposable {
    ...
    constructor(
        ...
        this._iconDecorationType = vscode.window.createTextEditorDecorationType(
            {
                after: {
                    ...
                    margin: "0 0 0 2em",
                    textDecoration:
                        "none; font-family: codicon; position: absolute; "
                }
            }
        );
        ...
        this._disposables.push(
            vscode.commands.registerCommand(
                PerformanceDecorator.Commands.Show,
                this.onShow.bind(this)
            )
        );
        this._disposables.push(
            vscode.commands.registerCommand(
                PerformanceDecorator.Commands.Hide,
    ...
    private getTotalDurationFromInsights(
        docInfo: DocumentInfo,
        method: MethodInfo
    ): number | undefined {
        const insights = docInfo.insights
            .forMethod(method, this._workspaceState.environment)
            .filter((x) => x.name === "Performance Stats");

        const percentileInfo = insights
            .map((x) => x as SpanDurationsInsight)
            .flatMap((x) => x.percentiles)
            .filter((x) => x.currentDuration);

        if (percentileInfo.length === 0) {
            return;
        }

        const totalDuration = percentileInfo
            .filter((x) => x.percentile == 0.5)
            .map((x) => x.currentDuration.raw)
    ...
    private async discoverIndirectDurationRecursive(
        ...
        for (const func of functions.filter((x) =>
            ...
            if (
                ...
                const completeDuration = this.getTotalDurationFromInsights(
                    docInfo,
                    methodInfo
                );
                if (completeDuration) {
            ...
            let totalDuration = this.getTotalDurationFromInsights(
                remoteDoc,
                remoteMethodInfo
            );
            if (!totalDuration) {
                const nextDoc = await vscode.workspace.openTextDocument(
                    remoteDoc.uri
                );
                if (nextDoc) {
                    totalDuration =
                        await this.discoverIndirectDurationRecursive(
                            nextDoc,
                            remoteDoc,
                            remoteMethodInfo,
                            remoteDoc.tokens.filter(
                                (x) => x.type === "function"
                            ),
                            recursionDepth++
                        );
    ...
    private async discoverPerfDecorators(
        ...
        for (const func of functions.filter((x) =>
            ...
            if (
                ...
                totalDuration = this.getTotalDurationFromInsights(
                    documentInfo,
                    methodInfo
                );
            } else {
                ...
                if (mem) {
                    ...
                    if (!totalDuration) {
                        ...
                        if (remoteTextDoc) {
                            ...
                            if (totalDuration) {
                                memoised[document.uri + "$$" + func.text] =
                                    totalDuration;
                            }
                        }
                    }
    ...
    private async onShow() {
        ...
        for (const method of docInfo.methods) {
            await this.discoverPerfDecorators(
                decorators,
                editor.document,
                docInfo,
                method,
    ...
    private addPerformanceDecorator(
        ...
        for (let exception of lineInfo.exceptions) {
            ...
            if (!exception.handled) {
                markdown.appendMarkdown(
                    ` \u00B7 <span style="color:#f14c4c;"><i>Unhandled</i></span>`
                );
            }
            if (exception.unexpected) {
                markdown.appendMarkdown(
                    ` \u00B7 <span style="color:#cca700"><i>Unexpected</i></span>`
                );
            }
    ...
    public dispose() {
        for (let dis of this._disposables) dis.dispose();
    }
}
