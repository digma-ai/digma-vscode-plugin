...
export class AnalyticsCodeLens implements vscode.Disposable {
    ...
    constructor(
        ...
        condeInspector: CodeInspector
    ) {
        this._provider = new CodelensProvider(
        ...
        this._disposables.push(
            vscode.commands.registerCommand(
                CodelensProvider.clickCommand,
                async (methodInfo: MethodInfo, environment: string) => {
                    if (vscode.window.activeTextEditor) {
                        if (
                            methodInfo.documentUri !==
        ...
        this._disposables.push(
            vscode.languages.registerCodeLensProvider(
                documentInfoProvider.symbolProvider.languageExtractors.map(
                    (x) => x.documentFilter
                ),
                this._provider
            )
...
class CodelensProvider implements vscode.CodeLensProvider<vscode.CodeLens> {
    ...
    private async getNoDataCodeLens(
        methodInfo: MethodInfo,
        codeObjectInfo: CodeObjectLocationInfo
    ): Promise<vscode.CodeLens[]> {
        const lens: vscode.CodeLens[] = [];
        lens.push(
            new vscode.CodeLens(codeObjectInfo.range, {
                title: "Never reached",
    ...
    private async getRuntimeDataExistsLens(
        methodInfo: MethodInfo
    ): Promise<vscode.CodeLens> {
    ...
    public async getDuplicateSpanLens(
        methodInfo: MethodInfo,
        codeObjectInfo: CodeObjectLocationInfo[]
    ): Promise<vscode.CodeLens[]> {
        return codeObjectInfo.map(
            (co) =>
                new vscode.CodeLens(co.range, {
                    title: "❗️ Duplicate span",
                    tooltip:
                        "A duplicate span was detected in this document, please change either span's name to avoid confusion",
                    command: CodelensProvider.clickCommand,
                    arguments: [methodInfo]
                })
        );
    }
    public async getLensForCodeLocationObject(
        methodInfo: MethodInfo,
        codeObjects: CodeObjectLocationInfo[],
        usageStatus: UsageStatusResults,
        allInsights: CodeObjectInsight[]
    ): Promise<vscode.CodeLens[]> {
        const codelens: vscode.CodeLens[] = [];

        const relevantCodeObjects = codeObjects.filter(
        ...
        for (const codeObject of relevantCodeObjects) {
            const insights = allInsights.filter(
                (x) => x.codeObjectId == codeObject.id
            );
            const codeObjectUsage = usageStatus.codeObjectStatuses.filter(
                (x) => x.codeObjectId == codeObject.id
            );
            if (insights.length == 0 && codeObjectUsage.length == 0) {
                const emptyLenses = await this.getNoDataCodeLens(
                    methodInfo,
                    codeObject
                );
            ...
            const currentEnvInsights = insights.filter(
                (x) => x.environment == this._state.environment
            );

            const lenses = await this.getCodeLens(
                methodInfo,
                codeObject,
                currentEnvInsights,
                false
            ...
            const otherEnvsInsightsToShow = insights
                .filter((x) => x.environment != this._state.environment)
                .filter(
                    (x) =>
                        x.decorators &&
                        x.importance < InsightImportance.important
                )
                .filter(
                    (x) =>
                        !currentEnvInsights.some(
                            (i) =>
                                i.type == x.type && i.importance == x.importance
                        )
                );

            const otherEnvLenses = await this.getCodeLens(
                methodInfo,
                codeObject,
                otherEnvsInsightsToShow,
                true
            );
    ...
    public async getCodeLensesForFunctions(
        document: vscode.TextDocument,
        documentInfo: DocumentInfo,
        token: vscode.CancellationToken
    ): Promise<vscode.CodeLens[]> {
        const memoised: { [token: string]: CodeLensData } = {};
        const functions = documentInfo.tokens.filter(
            (x) => x.type === "function"
        );
        const lens: vscode.CodeLens[] = [];
        for (const methodInfo of documentInfo.methods) {
            for (const func of functions.filter((x) =>
                x.range.intersection(methodInfo.range)
            )) {
                const funcKey = tokenKey(func.text, document.uri);
                if (funcKey in memoised) {
                    const data = memoised[funcKey];
                    const newLens = await this.getCodeLens(
                        data.methodInfo,
                        getFakeCodeLocation(func),
                        data.insights,
                        false
                    );
                    lens.push(...newLens);
                    ...
                    if (
                        func.text === methodInfo.name &&
                        func.range === methodInfo.nameRange
                    ) {
                        continue;
                    }
                    const remoteMethodInfo =
                        await this._codeInspector.getExecuteDefinitionMethodInfo(
                            document,
                            func.range.start,
                    ...
                    const remoteDoc = await this._codeInspector.getDocumentInfo(
                        document,
                        func.range.start,
                        this._documentInfoProvider
                    );
                    ...
                    const insights = remoteDoc.insights.forMethod(
                        remoteMethodInfo,
                        this._state.environment
                    );
                    const funcLens = await this.getCodeLens(
                        remoteMethodInfo,
                        getFakeCodeLocation(func),
                        insights,
                        false
        ...
        function getFakeCodeLocation(func: Token): CodeObjectLocationInfo {
            return {
                range: func.range,
                documentUri: documentInfo.uri,
                ids: [],
                displayName: func.text,
                id: func.text,
                idsWithType: [func.text]
            };
        }
    ...
    public async provideCodeLenses(
        ...
        const documentInfo = await this._documentInfoProvider.getDocumentInfo(
            document
        );
        if (!documentInfo) {
            return [];
        ...
        for (const methodInfo of documentInfo.methods) {
            ...
            for (const alias of methodInfo.ids) {
                const insights = documentInfo.insights.all.filter(
                    (x) => x.codeObjectId == alias
                );

                const thisEnvInsights = insights.filter(
                    (x) => x.environment == this._state.environment
                );
                ...
                if (methodCodeLens.length === 0 && thisEnvInsights.length > 0) {
                    methodCodeLens.push(
                        await this.getRuntimeDataExistsLens(methodInfo)
                    );
                }
            }

            let spans = documentInfo.spans.filter(
                (e) => e.range.intersection(methodInfo.range) != undefined
            );
            const duplicates = spans.filter((x) =>
                documentInfo.spans.some(
                    (span) =>
                        span != x &&
                        span.name == x.name &&
                        span.range != x.range
            ...
            if (duplicates.length > 0) {
                const lenses = await this.getDuplicateSpanLens(
                    methodInfo,
                    duplicates
                );
            ...
            if (spans.length > 0) {
                const lenses = await this.getLensForCodeLocationObject(
                    methodInfo,
                    spans,
                    documentInfo.usageData.getAll(),
                    documentInfo.insights.all.filter((x) => x.scope == "Span")
                );
            ...
            const endpoints = documentInfo.endpoints.filter(
                (e) => e.range.intersection(methodInfo.range) != undefined
            );
            const uniqueEndpoints = [
                ...new Map(endpoints.map((item) => [item.id, item])).values()
            ];
            if (uniqueEndpoints.length > 0) {
                const lenses = await this.getLensForCodeLocationObject(
                    ...
                    uniqueEndpoints,
                    documentInfo.usageData.getAll(),
                    documentInfo.insights.all.filter(
                        (x) => x.scope == "EntrySpan" || x.scope == "Span"
                    )
                );
            ...
            for (const alias of methodInfo.ids) {
                ...
                const otherEnvsInsights = insights
                    .filter((x) => x.environment != this._state.environment)
                    .filter(
                        (x) =>
                            x.decorators &&
            ...
            const uniqueLenses = [
                ...new Map(
                    methodCodeLens.map((item) => [item.command!.title, item])
                ).values()
            ];
